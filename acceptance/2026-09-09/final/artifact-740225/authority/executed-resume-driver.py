...
root=Path('/tmp/chio-codex-final-authority-evidence-20260909');root.mkdir(mode=0o700,exist_ok=True)
...
cases=['budget','wrong-server'];configs={name:json.loads((root/f'{name}-config.private.json').read_text()) for name in cases}
def budget(label):
