#!/usr/bin/env bash
# verify-release.sh — verify a published @chio/codex-plugin release.
#
# Usage:
...
PKG="@chio/codex-plugin"
SOURCE_REPO="${CHIO_GH_OWNER:-owner}/chio-codex-plugin"
...
if [[ -n "$FIXTURE" && -f "$FIXTURE" ]]; then
  ...
  tarball="chio-codex-plugin-${VERSION#v}.tgz"
  : > "$tmp/$tarball"
else
