...
root=Path('/tmp/chio-codex-tool-discovery-evidence-20260909');root.mkdir(mode=0o700)
...
class Handler(http.server.BaseHTTPRequestHandler):
 ...
 def do_POST(self):
  global requests
  request=json.loads(self.rfile.read(int(self.headers.get('Content-Length','0'))));requests+=1;(root/f'request-{requests}.json').write_text(json.dumps({'tools':request.get('tools'),'input':request.get('input')},indent=2)+'\n')
  for item in request.get('input',[]):
   if item.get('type')=='function_call_output':results[item.get('call_id')]=item.get('output')
  if requests==1:item={'type':'tool_search_call','id':'ts_probe','call_id':'ts_probe','status':'completed','execution':'client','arguments':{'query':'chio read_text_file','limit':1}}
