...
globalThis.fetch = async function(input, init) {
  ...
  response.text = async function() {
    ...
    const finished = performance.now();
    record({ stage: 'kernel-fetch-to-full-body', startedAtEpochMs: epoch,
      ...
      arguments: request.params?.arguments, status: response.status,
      excludes: ['model', 'launcher startup', 'bridge post-response verification', 'delivery ACK'],
      includes: ['HTTP transport', 'kernel authorization and durable execution', 'resource server', 'receipt signing and persistence', 'full response body consumption'] });
    return body;
  };
  return response;
};
