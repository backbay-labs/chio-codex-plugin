...
This lane builds `@chio/codex-plugin` from one immutable source commit. A passing workflow
...
- Public repository identity: `backbay-labs/chio-codex-plugin`.
- Workflow: `.github/workflows/release.yml`.
- Registry package: `@chio/codex-plugin`; GitHub environment: `npm`.
...
5. An npm maintainer must register this package's Trusted Publisher with GitHub
   owner `backbay-labs`, repository `chio-codex-plugin`, workflow filename `release.yml`,
...
cosign verify-blob --certificate SHA256SUMS.pem --signature SHA256SUMS.sig \
  --certificate-identity 'https://github.com/backbay-labs/chio-codex-plugin/.github/workflows/release.yml@refs/tags/v0.3.0' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com SHA256SUMS
slsa-verifier verify-artifact package.tgz \
  --provenance-path package.intoto.jsonl \
  --source-uri github.com/backbay-labs/chio-codex-plugin --source-tag 'v0.3.0'
```
