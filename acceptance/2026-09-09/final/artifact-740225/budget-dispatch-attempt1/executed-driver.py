...
root=Path('/tmp/chio-codex-budget-dispatch-evidence-20260909');root.mkdir(mode=0o700)
installed=Path(Path('/tmp/chio-codex-final-install-pointer.txt').read_text());artifact=json.loads((installed/'artifact.json').read_text());package=Path(artifact['prefix'])/'node_modules/@chio/codex-plugin';gateway=package/'node_modules/@chio/bridge/dist/gateway.js'
prior=Path('/tmp/chio-codex-final-authority-evidence-20260909/budget-config.private.json');config=json.loads(prior.read_text());config['sessionId']='codex-budget-driver-continuation-20260909';config['journalDir']=str(root/'journal');configpath=root/'config.private.json';configpath.write_text(json.dumps(config));configpath.chmod(0o600)
