...
def native(name, tool, arguments):
    ...
    assert len(timings) == 2 and {r['stage'] for r in timings} == {'kernel-fetch-to-full-body', 'native-gateway-request-to-response-end'}
...
with (a.output / 'direct.stderr').open('w') as stderr:
    ...
    try:
        ...
        for number in range(1, 4):
            ...
            kernel = next(row['elapsedMs'] for row in timings if row['stage'] == 'kernel-fetch-to-full-body')
            pair = {'pair': number, 'order': 'direct then native' if number == 2 else 'native then direct',
                'nativeGatewayRequestToResponseEndMs': gateway, 'kernelFetchToFullBodyMs': kernel,
