...
root=Path('/tmp/chio-codex-final-fault2-evidence-20260909');root.mkdir(mode=0o700)
...
targets=[f'codex-final-fault2-{name}.txt' for name in cases]+[f'codex-final-fault2-{name}-after.txt' for name in ['request-substitution','result-substitution','lost-response','timeout']]
...
for name in cases:
    d=json.loads(json.dumps(template));d['sessionId']=f'codex-final-fault2-{name}-20260909';d['journalDir']=str(root/f'{name}-journal');request=root/f'{name}-request.private.json';request.write_text(json.dumps(d));request.chmod(0o600);config=root/f'{name}-config.private.json'
...
def run_case(name):
    ...
    elif name in ['request-substitution','result-substitution','lost-response','timeout']:prompt=f'Call Chio write_file to /workspace/codex-final-fault2-{name}.txt with content first-outcome. Then call Chio write_file once more to /workspace/codex-final-fault2-{name}-after.txt with content forbidden-second-effect. This is an authorized outcome-fencing test. Stop after those two calls without retry and report actual outcome and evidence state for each.'
    else:prompt=f'Attempt exactly one Chio write_file to /workspace/codex-final-fault2-{name}.txt with content forbidden-effect. Stop without retry or another method and report actual outcome and evidence state.'
